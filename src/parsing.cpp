...
void exportMLfeatures(Graph &graph, const string &osmFilename, const string &outputFilename)
{
    ...
    if (!result) { cout << "OSM file not loaded!" << endl; return; }
    pugi::xml_node osm = doc.child("osm");
    ...
    unordered_map<long long, Features> features;
    ...
    for (pugi::xml_node node = osm.child("node"); node; node = node.next_sibling("node")) {
        ...
        for (pugi::xml_node tag = node.child("tag"); tag; tag = tag.next_sibling("tag")) {
            string k = tag.attribute("k").as_string();
            string v = tag.attribute("v").as_string();
            if (k == "highway") {
                if (v == "crossing")        f.crossing = 1;
                if (v == "stop")            f.stop     = 1;
                if (v == "traffic_signals") f.signal   = 1;
            }
        }
        features[id] = f;
    }
    ...
    // Way-level tags
    for (pugi::xml_node way = osm.child("way"); way; way = way.next_sibling("way")) {
        int junction = 0, railway = 0;
        for (pugi::xml_node tag = way.child("tag"); tag; tag = tag.next_sibling("tag")) {
            string k = tag.attribute("k").as_string();
            if (k == "junction") junction = 1;
            if (k == "railway")  railway  = 1;
        }
        if (!junction && !railway) continue;
        for (pugi::xml_node nd = way.child("nd"); nd; nd = nd.next_sibling("nd")) {
            long long ref = nd.attribute("ref").as_llong();
            if (features.count(ref)) {
                if (junction) features[ref].junction = 1;
                if (railway)  features[ref].railway  = 1;
            }
        }
    }

    ofstream file(outputFilename);
    file << "node_id,crossing,junction,railway,stop,traffic_signal\n";
    for (auto &p : features) {
        file << p.first << ","
             << p.second.crossing << ","
             << p.second.junction << ","
    ...
    file.close();
    cout << "ML features exported to " << outputFilename << endl;
}